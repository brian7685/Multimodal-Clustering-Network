...
class Youtube_DataLoader(Dataset):
    ...
    def _get_single_audio_text(self, caption, ind, mel_spec):
        ...
        frames = librosa.core.time_to_frames([caption['start'][start], caption['end'][end]], sr=16000, hop_length=160,
                                             n_fft=400)
        padded_mel_spec, nframes = self._zero_pad_audio(mel_spec[:, frames[0]: frames[1]], self.num_audio_frames)
        return th.from_numpy(padded_mel_spec), nframes, caption['start'][start], caption['end'][end], self._words_to_we(
            words)
    ...
    def __getitem__(self, idx):
        ...
        if self.random_audio_windows:
            return {'video': video, 'audio': th.HalfTensor(audio), 'nframes': th.IntTensor(nframes),
                    'video_id': video_id}
        else:
            return {'video': video, 'audio': th.HalfTensor(audio), 'nframes': th.IntTensor(nframes),
                    'video_id': video_id,
...
class Youtube_DataLoader_label(Dataset):
    """Youtube dataset loader."""
    ...
    def __init__(
        ...
        if features_path != '':
            self.feature_path['3d'] = features_path
        self.random_audio_windows = random_audio_windows
        self.pseu_label_a = pseu_label_a
        self.pseu_label_v = pseu_label_v
    ...
    def __getitem__(self, idx):
        ...
        if self.random_audio_windows:
            ...
            return {'video': video, 'audio': th.HalfTensor(audio), 'nframes': th.IntTensor(nframes),
                    'video_id': video_id,
                    'text': text, 'pseudo_v': self.pseudo_v[idx], 'pseudo_a': self.pseudo_a[idx]}