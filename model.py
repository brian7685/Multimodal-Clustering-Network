...
class Net(nn.Module):
    ...
    def forward(self, video, text):
        video = self.GU_video(video) #[frames,emb]
        text = self.GU_text(self.text_pooling(text)) #[class,emb]
        if self.mil==0:
            return th.matmul(text, video.t()) #[class,frame]
        else:
            return video, text
